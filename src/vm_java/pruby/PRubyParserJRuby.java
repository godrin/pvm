...
/*
	...
	@Override
	public String parse(PRubySourceDef def) {
		byte[] pRubySource = def.getData();
		...
		String parserSource;
		try {
			parserSource = FileTool.loadFileString(new File(PARSER_RB));
		} catch (IOException e) {
			VMLog.error(e);
			return null;
		}
		...
		try {
			...
			if (useEngine) {
				...
				System.out.println("A");
				Ruby r = Ruby.newInstance(ric);
				System.out.println("A");
				String s = r.evalScriptlet(parserSource);
				System.out.println("A");
